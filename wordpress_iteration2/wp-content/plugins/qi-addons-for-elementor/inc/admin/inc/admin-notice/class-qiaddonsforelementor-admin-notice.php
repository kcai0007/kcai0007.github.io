...
class QiAddonsForElementor_Admin_Notice {
	...
	// Used to determine minimum number of days after which review notice will show up again if customer clicked "Already Did" option
	private $review_already_did_postponement_days_min = 10;

	// Used to determine maximum number of days after which review notice will show up again if customer clicked "Already Did" option
	private $review_already_did_postponement_days_max = 15;
	...
	public function add_review_notice() {
		...
		if ( $plugin_duration_in_seconds > $minimal_duration_for_review_notice ) {
			...
			if ( ! empty( $review_option ) && 'reviewed' !== $review_option ) {
	...
	public function handle_review_notice() {
		...
		if( 'already-reviewed' === $review_action ) {
			$postponement_buffer = $this->geneterate_number_of_seconds_from_days( rand( $this->review_already_did_postponement_days_min, $this->review_already_did_postponement_days_max) );
		}
