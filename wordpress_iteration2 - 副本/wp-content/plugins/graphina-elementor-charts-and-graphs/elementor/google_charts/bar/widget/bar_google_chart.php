...
class Bar_google_chart extends Widget_Base
{
    ...
    {
//        require GRAPHINA_ROOT . '/elementor/google_charts/bar/render/bar_google_chart.php';
        $mainId = $this->get_id();
        $type = $this->get_chart_type();
        $settings = $this->get_settings_for_display();
        $seriesListCount = $settings['iq_' . $type . '_chart_data_series_count'];
        $barData = $valueList = $element_colors = $seriesStyleArray = $elementTitleArray = [];
        ...
        if($settings['iq_' . $type . '_chart_data_option'] === 'manual'){
            ...
            foreach ($categoryList as $key => $value) {
                ...
                array_push($barData, $temp);
            }
        }

        $barDataElementCount = $settings['iq_' . $type . '_chart_data_series_count'];

        for ($j = 0; $j < $barDataElementCount; $j++) {
            ...
            $label_count = count($barData);
            if($label_count < $valueListCount){
                array_push($barData, ['Label']);
            ...
            if(!empty($valueList) && count($valueList) > 0){
                foreach ($valueList as $key => $value) {
                    array_push( $barData[$key], $value['iq_'.$type.'_chart_value_3_'. $j]);
                    ...
                    if($annotationShow === 'yes'){
                        if( !empty($settings['iq_' . $type . '_chart_annotation_prefix_postfix']) ){
                            ...
                            array_push( $barData[$key], $annotationPrefix.$val.$annotationPostfix );
                        }else{
                            array_push( $barData[$key], $val );
                        }
                    }
                }
            }
        ...
        if(!empty($settings['iq_' . $type . '_chart_haxis_label_prefix_postfix'])){
            ...
            $barDataLength = count($barData);
            for ($x = 0; $x < $barDataLength; $x++) {
                $barData[$x][0] = $xPrefix.$barData[$x][0].$xPostfix;
            }
        }
        ...
        <script type="text/javascript">
            ...
            function drawBarColors() {
                ...
                data.addRows(<?php print_r($barData); ?>);
                ...
                var options = {
                    ...
                    hAxis: {
                        ...
                        format:'<?php echo $settings['iq_' . $type . '_chart_vaxis_format']; ?>',
                ...
                var chart = new google.visualization.BarChart(document.getElementById('bar_google_chart<?php esc_attr_e($this->get_id()); ?>'));
