...
class Area_google_chart extends Widget_Base
{
    ...
    {
//        require GRAPHINA_ROOT . '/elementor/google_charts/area/render/area_google_chart.php';

        $mainId = $this->get_id();
        $type = $this->get_chart_type();
        $settings = $this->get_settings_for_display();
        $seriesListCount = $settings['iq_' . $type . '_chart_data_series_count'];
        $areaData = $valueList = $element_colors = $seriesStyleArray = $elementTitleArray = [];
        $animatioShow = $settings['iq_' . $type . '_chart_animation_show'];
        $animatioShowValue = $animatioShow == 'yes' ? 'true' : 'false';
        $hAxisdirectionShow = $settings['iq_' . $type . '_chart_haxis_direction'];
        $hAxisdirectionShowValue = $hAxisdirectionShow == 'yes' ? -1 : 1;
        ...
        if( ($settings['iq_' . $type . '_chart_stacked_show']) === 'yes' ){
            $stackedShow = true;
        }else{
            $stackedShow = false;
        }
        ...
        $areaDataElementCount = $settings['iq_' . $type . '_chart_data_series_count'];

        for ($j = 0; $j < $areaDataElementCount; $j++) {
            ...
            $serire_array = ['lineWidth' => $lineWidth, 'lineDashStyle' => $lineDash, 'pointShow' => $pointShow, 'pointSize' => $pointSize, 'pointShape' => $pointShape, 'targetAxisIndex' => $targetAxisIndex];
            ...
            $min_value = !empty($val_array) ? min($val_array) : '';
            $max_value = !empty($val_array) ? max($val_array) : '';

            // ---- Inconsistent data length fix ----
            $label_count = count($areaData);
            if($label_count < $valueListCount){
                array_push($areaData, ['Label']);
            ...
            if(!empty($valueList) && count($valueList) > 0){
                foreach ($valueList as $key => $value) {
                    array_push( $areaData[$key], $value['iq_'.$type.'_chart_value_3_'. $j] );
                    ...
                    if( $annotationShow === 'yes' ){
                        if( !empty($settings['iq_' . $type . '_chart_annotation_prefix_postfix']) ){
                            ...
                            array_push( $areaData[$key], $annotationPrefix.$val.$annotationPostfix );
                        }else{
                            array_push( $areaData[$key], $val );
                        }
                    }
                }
            }
        ...
        if(!empty($settings['iq_' . $type . '_chart_haxis_label_prefix_postfix'])){
            ...
            $areaDataLength = count($areaData);
            for ($x = 0; $x < $areaDataLength; $x++) {
                $areaData[$x][0] = $xPrefix.$areaData[$x][0].$xPostfix;
            }
        }
        ...
        <script type='text/javascript'>
            ...
            function drawChart() {
                ...
                var hAxisdirectionShowValue = '<?php echo $hAxisdirectionShowValue; ?>';
                ...
                var stackedShow = '<?php echo $stackedShow; ?>';
                ...
                var data = new google.visualization.DataTable();
                    ...
                    data.addRows(<?php print_r($areaData); ?>);
                ...
                var options = {
                    ...
                    hAxis: {
                        ...
                        direction: hAxisdirectionShowValue,
                ...
                var chart = new google.visualization.AreaChart(document.getElementById('area_google_chart<?php esc_attr_e($this->get_id()); ?>'));
