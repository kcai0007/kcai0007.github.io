...
//                    ]
    ...
    {
//        require GRAPHINA_ROOT . '/elementor/google_charts/pie/render/pie_google_chart.php';
        ...
        if($settings['iq_' . $type . '_chart_isthreed'] == 'yes'){
            $is3d = true;
        }else{
            $is3d = false;
        }
        ...
        //label  _chart_pieSliceText
        if( ($settings['iq_' . $type . '_chart_pieSliceText_show']) === 'yes' ){
            $labelPosition = $settings['iq_' . $type . '_chart_pieSliceText'];
        }else{
            $labelPosition = 'none';
        }
        ...
        switch ($dataTypeOption) {
            case "manual":
                for ($i = 0; $i < $valueList; $i++) {
                    ...
                    array_push( $pieData, $new_list);
                }
                break;
            case "csv":
                if(isGraphinaPro()){
                    $data = graphina_pro_parse_csv($settings, $type, 'circle');
                }
                break;
            case "remote-csv" :
            case "google-sheet" :
                if(isGraphinaPro()){
                    $data = graphina_pro_get_data_from_url($type, $settings, $dataTypeOption, $mainId, 'circle');
                }
                break;
            case "api":
                if(isGraphinaPro()){
        ...
        if(!empty($settings['iq_' . $type . '_chart_label_prefix_postfix'])){
            $labelPrefix = $settings['iq_' . $type . '_chart_label_prefix'];
            $labelPostfix = $settings['iq_' . $type . '_chart_label_postfix'];
            $pieDataLength = count($pieData);
            for ($x = 0; $x < $pieDataLength; $x++) {
                $pieData[$x][0] = $labelPrefix.$pieData[$x][0].$labelPostfix;
            }
        }
    ...
    ?>
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                ...
                data.addRows(<?php print_r($pieData); ?>);
                ...
                var labelPosition = '<?php echo strval($labelPosition); ?>';
                ...
                var chart = new google.visualization.PieChart(document.getElementById('pie_google_chart<?php esc_attr_e($this->get_id()); ?>'));
