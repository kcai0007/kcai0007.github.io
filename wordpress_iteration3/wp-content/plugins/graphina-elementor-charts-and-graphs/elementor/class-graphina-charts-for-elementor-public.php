...
class Graphina_Charts_For_Elementor_Public
{
    ...
    private $helper;
    private $all_chart = ['area_chart', 'bubble_chart', 'candle_chart', 'column_chart', 'donut_chart', 'heatmap_chart', 'line_chart', 'pie_chart', 'polar_chart', 'radar_chart', 'radial_chart', 'timeline_chart', 'mixed_chart', 'nested_column_chart', 'graphina_counter', 'counter_chart'];

    private $google_charts = ['line_google_chart', 'area_google_chart','pie_google_chart' ,'column_google_chart','bar_google_chart', 'donut_google_chart', 'geo_google_chart', 'gauge_google_chart' ];
    ...
    public function enqueue_scripts($id = 0)
    {
        ...
        if (graphina_is_preview_mode()) {
            ...
            if( (in_array("apex_chart_js", $selected_js_array)) && (count(array_intersect($get_graphina_widget, $this->all_chart)) > 0) ) {
                wp_enqueue_script('apexcharts-min', plugin_dir_url(__FILE__) . 'js/apexcharts.min.js', array('jquery'), $this->version, false);
                // wp_enqueue_script('datatable-min', 'https://cdn.datatables.net/1.11.4/js/jquery.dataTables.min.js', array('jquery'), $this->version, false);
            }
            if( (in_array("google_chart_js", $selected_js_array)) && (count(array_intersect($get_graphina_widget, $this->google_charts)) > 0) ) {
            ...
            //     wp_enqueue_script('apexcharts-min', plugin_dir_url(__FILE__) . 'js/apexcharts.min.js', array('jquery'), $this->version, false);
            // }
            // if (count(array_intersect($get_graphina_widget, $this->google_charts)) > 0) {
            //     wp_enqueue_script('googlecharts-min', plugin_dir_url(__FILE__) . 'js/gstatic/loader.js', [], $this->version, false);
            // }
    ...
    public function check_required_plugins_for_graphina()
    {
        if ($this->graphina_is_elementor_installed()) {
            if (!is_plugin_active('elementor/elementor.php')) {
                ...
                $message = __('<strong>Graphina - Elementor Charts and Graphs</strong> requires <strong>Elementor</strong> plugin to be active. Please activate Elementor for Graphina - Elementor Charts and Graphs to continue.', 'graphina-charts-for-elementor');
                $button_text = __('Activate Elementor ', 'graphina-charts-for-elementor');
            ...
            $message = __('<strong>Graphina - Elementor Charts and Graphs</strong> Not working because you need to install the <strong>Elementor</strong> plugin.', 'graphina-charts-for-elementor');
            $button_text = __('Install Elementor ', 'graphina-charts-for-elementor');
