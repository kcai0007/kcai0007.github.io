    ...
    protected function render()
    {
        ...
        if (isRestrictedAccess('donut', $this->get_id(), $settings, false) === false) {
            ...
            <script>
                ...
                var donutOptions = {
                    series: '<?php  echo   esc_html($value); ?>'.split('_,_').map((val) => parseFloat(val)),
                    chart: {
                ...
                if("<?php echo $settings['iq_' . $type . '_chart_datalabels_format'] === 'yes'?>"){
                    ...
                    if(typeof chartDatalabelsFormat !== "undefined"){
                        chartDatalabelsFormat(donutOptions, showlabel, showValue, numberformat, prefix, postfix ,valueInStringShow, valueInStringValue);
                    }
                }
...
Plugin::instance()->widgets_manager->register(new \Elementor\Donut_chart());